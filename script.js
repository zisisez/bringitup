...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_1__["default"]('.module__video-item .play', '.overlay').init(); // Еще один способ вызова конструктора
...
function () {
  function Difference(oldOfficer, newOfficer, items) {
    ...
    try {
      // Забираем две колонки 
      this.oldOfficer = document.querySelector(oldOfficer);
      this.newOfficer = document.querySelector(newOfficer); // Забираем содержимое колонок

      this.oldItems = this.oldOfficer.querySelectorAll(items);
      this.newItems = this.newOfficer.querySelectorAll(items);
      this.items = items; //Для каждой колонки задаем отдельный счетчик
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers(container, items, counter) {
      // Находим кнопку открытия контента в колонке
      container.querySelector('.plus').addEventListener('click', function () {
        //Если счетчик не равен длине массива минус 2, то по индексу показываем элемент , у нас есть 4 элемента массива и два из них должны быть показаны (Тайтл и кнопка добавления контента) Так вот если их не два мы показываем элемент и плюсуем счетчик
        if (counter !== items.length - 2) {
          items[counter].style.display = 'flex';
          counter++;
          console.log(items.length);
        } else {
          // Иначе показываем элемент и удаляем последний айтем она же кнопка добавления(если показаны все части колонки)
          items[counter].style.display = 'flex';
    ...
    value: function hideItems(items) {
      items.forEach(function (item, i, arr) {
        // Если индекс не равен последнему элементу то скрываем 
        if (i !== arr.length - 1) {
...
function () {
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      //Забираем все кнопки и вешаем событие 
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            _this.activeBtn = btn; //Если видео было открыто API пытается создать новое
            ...
            if (document.querySelector('iframe#frame')) {
              ...
              //Иначе создаем с API новое видео, забирая ссылку из верстки
              _this.path = btn.getAttribute('data-url');
    ...
    } //Функция закрытия видео с остановкой воспроизведения (паузой)
    ...
    } //Создаем видео из API youtube , по ссылке которую передаем из верстки
    ...
    value: function onPlayerStateChange(state) {
      try {
        //closest - получаем первую ноду выше по иерархии от переданного селектора 
        //nextElementSibling - получаем соседний элемент 
        //cloneNode(true) - копируем ноду, если тру то и  ее детей (глубокое копирование)
    ...
    } // Инициализирум плеер из API (настройка из документации youtube iframe api)
    ...
    value: function init() {
      if (this.btns.length > 0) {
        ...
        firstScriptTag.parentNode.insertBefore(tag, firstScriptTag); //И запускаем нужные функции
...
function (_Slider) {
  ...
  function MainSlider(btns) {
    ...
    //Забираем из прототипа нужные нам аргументы
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      } // Если элемент присутсвует и код может выполниться то (используем этот функционал если предполагаем что код может не выполниться) тогда скрипт не сломается и продолжит работу
    ...
    } // Показываем необходимый элемент через время если переходим на нужную страницу
    ...
    value: function render() {
      if (this.container) {
        ...
        } catch (e) {} // Выполняем первичную инициализацию слайдера


        this.showSlides(this.slideIndex); // Инициализируем функцию показа блока через время
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        //Изначально удаляем у всех элементов массива все активные классы
        slide.classList.remove(_this.activeClass); // Если слайд имет свойство анимейт то задаем изначальные параметры

        if (_this.animate) {
          slide.querySelector('.card__title').style.opacity = '0.4';
          slide.querySelector('.card__controls-arrow').style.opacity = '0';
        }
      }); // Если первый активный слайд не является кнопкой то мы добавляем класс активности

      if (!this.slides[0].closest('button')) {
        this.slides[0].classList.add(this.activeClass);
      } // Если активный слайд будет иметь свойство анимейт то меняем параметры
    ...
    value: function bindTriggers() {
      ...
      // При клике на кнопку Некст запускаем функцию которая проверит все слайды и изменит нужные параметры 
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== "BUTTON") {
            //Определяем последний слайд
            var active = _this2.slides[i]; //Помещаем последний слайд перед первым 1й аргумент сам слайд, 2й перед каким хотим поместить
...
var Slider = function Slider() {
  ...
  // Создаем новое свойство которое будет в экземпляре класса 
  this.container = document.querySelector(container); // Получаем всех детей основного блока слайдера 
